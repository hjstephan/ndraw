...
# Annahme: ndraw.py liegt im src/ Verzeichnis
...
class TestDirectedEdge:
    ...
    def test_edge_update_position(self):
    ...
    def test_edge_position_update_on_node_move(self):
...
class TestJSONImportExport:
    ...
    def test_json_export_structure(self, main_window, tmp_path):
        ...
        node1 = main_window.canvas.add_new_node(100, 200, 0)
        node2 = main_window.canvas.add_new_node(300, 400, 1)
        main_window.canvas.add_new_edge(node1, node2)
        
        # Exportiere zu JSON
        json_file = tmp_path / "test_network.json"
        main_window.canvas.nodes = [node1, node2]
        main_window.canvas.edges = [DirectedEdge(node1, node2)]
        
        data = {
            "nodes": [{"id": n.node_id, "x": n.pos().x(), "y": n.pos().y()} 
...
class TestSVGExport:
    ...
    def test_svg_export_file_creation(self, main_window, tmp_path):
        ...
        node2 = main_window.canvas.add_new_node(200, 200, 1)
        main_window.canvas.add_new_edge(node1, node2)
